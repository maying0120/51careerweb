function scoreChange(a){function e(){var b=document.getElementById("doughnutChartCanvas").getContext("2d");
new Chart(b).Doughnut(c,d),$("#resumeScore .scoreVal span").text(a).parent().fadeIn(200),100==a?$("#resumeScore .which span").html('<a href="'+ctx+'/jobs/list_所有职位">马上去投递</a>'):$("#resumeScore .which span").html("<a>马上去完善</a>")}var b=360*parseInt(a)/100,c=[{value:b,color:"#009a6d"},{value:360-b,color:"#ffffff"}],d={percentageInnerCutout:80,segmentShowStroke:!1,animationEasing:"easeOutQuart"};
setTimeout(e,300)}function changeAllIds(a){var c,d,e,f,b=JSON.parse(a);
$("#resumeId").val(b.resumeId.n),c=b.expId,d=b.eduId,e=b.projectId,f=b.showId,$("#workExperience .wlist li").each(function(){var b,a=$(this).attr("data-id");
for(b=0;
b<c.length;
b++)a==c[b].o&&$(this).attr("data-id",c[b].n)}),$("#projectExperience .plist li").each(function(){var b,a=$(this).attr("data-id");
for(b=0;
b<e.length;
b++)a==e[b].o&&$(this).attr("data-id",e[b].n)}),$("#educationalBackground .elist li").each(function(){var b,a=$(this).attr("data-id");
for(b=0;
b<d.length;
b++)a==d[b].o&&$(this).attr("data-id",d[b].n)}),$("#worksShow .slist li").each(function(){var b,a=$(this).attr("data-id");
for(b=0;
b<f.length;
b++)a==f[b].o&&$(this).attr("data-id",f[b].n)})}function openYear(a){$(".profile_select_normal").removeClass("select_focus"),$(".boxUpDown").hide(),$(".select_"+a).addClass("select_focus"),$(".box_"+a).show()}function selectYear(a,b){$(".select_"+a).val(b).css("color","#333").removeClass("select_focus"),$("."+a).val(b),$(".box_"+a).hide()}function suggestCompany(a,b){var c="";
$.ajax({type:"POST",async:!1,url:ctx+"/c/companySearchCompanyLs.json",data:{kd:a},dataType:"json"}).done(function(a){c=a}),$(b).autocomplete({source:c})}function suggestPosition(a,b){var c="";
$.ajax({type:"POST",async:!1,url:ctx+"/jobs/listLs",data:{pl:a},dataType:"json"}).done(function(a){a.success&&(c=a.content.result)}),$(b).autocomplete({source:c})}function click_scroll(a){var b=$("#"+a).offset();
switch(a){case"expectJob":$("#expectJob .expectAdd").trigger("click");
break;
...
break;
case"selfDescription":$("#selfDescription .descriptionAdd").trigger("click");
break;
case"worksShow":$("#worksShow .workAdd").trigger("click");
break;
default:return}$("body,html").animate({scrollTop:b.top},400,function(){$("#"+a).animate({borderColor:"#e46a4a"},300,function(){$(this).animate({borderColor:"transparent"})})})}function img_check(a,b,c){var a=$("#"+c);
this.AllowExt=".jpg,.gif,.jpeg,.png,.pjpeg",this.FileExt=a.val().substr(a.val().lastIndexOf(".")).toLowerCase(),0!=this.AllowExt&&-1==this.AllowExt.indexOf(this.FileExt)?errorTips("只支持jpg、gif、png、jpeg格式，请重新上传","上传头像"):($("#"+c+"_error").text("").hide(),$.ajaxFileUpload({url:b,secureuri:!1,fileElementId:c,data:{headPic:a.val()},dataType:"json",success:function(a){if(a.success){var b=a.content;
$("#portraitShow img").attr("src",ctx+"/"+b),$("#basicInfo .basicShow img").attr("src",ctx+"/"+b),$("#portraitNo").hide(),$("#portraitShow").show()}else errorTips("上传失败，请重新上传","上传头像")},error:function(){errorTips("上传失败，请重新上传","上传头像")}}))}function file_check(obj,action_url,id){var userId;
...
window.onbeforeunload=function(){return a?void 0:"内容还未保存，确认离开该页面吗？ "},$(".profile_radio li input").click(function(){$(this).parent("li").siblings("li").removeClass("current"),$(this).parent("li").addClass("current")}),$(document).click(function(){$(".boxUpDown").hide().prev("input").removeClass("select_focus")}),$("#resumeScore .which span").bind("click",function(){var a=$(this).attr("rel");
"complete"!=a&&click_scroll(a)}),jQuery.validator.addMethod("checkWorkYear",function(a,b){var c=$(b).parents(".experienceForm"),d=c.find(".companyYearStart").val(),e=c.find(".companyMonthStart").val(),f=c.find(".companyYearEnd").val(),g=c.find(".companyMonthEnd").val();
return d>f?!1:d==f&&e>g?!1:!0},"开始时间需早于结束时间"),jQuery.validator.addMethod("checkProjectYear",function(a,b){var c=$(b).parents(".projectForm"),d=c.find(".projectYearStart").val(),e=c.find(".projectMonthStart").val(),f=c.find(".projectYearEnd").val(),g=c.find(".projectMonthEnd").val();
return d>f?!1:d==f&&e>g?!1:!0},"开始时间需早于结束时间"),jQuery.validator.addMethod("checkSchoolYear",function(a,b){var c=$(b).parents(".educationalForm"),d=c.find(".schoolYearStart").val(),e=c.find(".schoolYearEnd").val();
return d>e?!1:!0},"开始时间需小于结束时间"),$("#resume_name .nameShow .rename").bind("click",function(){$(this).parent().addClass("dn"),$(this).parent().siblings("form").removeClass("dn"),j($("#resume_name"))}),$("#resumeNameForm").validate({rules:{resumeName:{required:!0,checkNum:!0,maxlenStr:100}},messages:{resumeName:{required:"请输入简历名称",checkNum:"请输入有效的简历名称",maxlenStr:"请输入100字以内的简历名称"}},errorPlacement:function(a,b){a.appendTo($(b).parent())},submitHandler:function(a){var b=$('input[name="resumeName"]',a).val(),c=$("#resumeId").val();
$(a).find(":submit").attr("disabled",!0),$.ajax({url:ctx+"/resume/renameResume.json",type:"POST",data:{resumeName:b,id:c}}).done(function(b){if(b.success){var c="";
c=b.content.resumeName.length>18?b.content.resumeName.substring(0,15)+"...":b.content.resumeName,$("#resume_name .nameShow h1").attr("title",b.content.resumeName).text(c).parent().removeClass("dn"),$("#resume_name form").addClass("dn"),$("#lastChangedTime span").text(b.content.refreshTime),k($("#resume_name"))}else alert(b.msg);
$(a).find(":submit").attr("disabled",!1)})}}),$("#basicInfo .c_edit").bind("click",function(){var a=$("#nameVal").val(),b=$("#genderVal").val(),c=$("#topDegreeVal").val(),d=$("#workyearVal").val(),e=$("#currentStateVal").val(),f=$("#emailVal").val(),g=$("#telVal").val();
...
}),$(".select_companyYearEnd").bind("click",function(a){a.stopPropagation(),openYear("companyYearEnd")}),$(".box_companyYearEnd").on("click","ul li",function(a){var b,c,d;
